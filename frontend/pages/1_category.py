...
#     st.warning("Please login firts from main page")
#     st.stop()
# token = st.session_state.token
# st.subheader("Create  New Category")
# name=st.text_input("Category Name (e.g. Backend Dev, ML)")
...
#             with col1:
#                 if st.button("Updtae",key=f'cat_update_{cat["id"]}'):
#                     update_category(token,new_name,cat["id"])
#                     st.success("Updated!")
#                     st.rerun()
#                 with col2:
#                     if st.button("Delete",key=f'cta_name_{cat["id"]}'):
